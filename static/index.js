...
function show_word() {
  ...
  pronounce.innerHTML = words[index]['pronounce']
  get_meanings()
}
...
function show_meaning(meaning) {
  ...
  pos.innerHTML = meaning['pos']
  meaning_ch.innerHTML = meaning['meaning_ch']
...
function show_meanings() {
  meanings.innerHTML = ""
  words[index]['meaning'].forEach(show_meaning)
  index = index + 1
}

function get_meanings() {
  if (words[index]['meaning'] == null || words[index]['meaning'].length == 0) {
    fetch("/getmeaning", {
      method: 'POST',
